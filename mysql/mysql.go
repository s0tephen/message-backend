...
func Init() error {
	db, err := dbConnect(config.Config.Mysql.User, config.Config.Mysql.Password, fmt.Sprintf("%s:%d", config.Config.Mysql.Host, config.Config.Mysql.Port), config.Config.Mysql.DB)
	if err != nil {
		return err
	}
	DB = &dbRepo{Db: db}
